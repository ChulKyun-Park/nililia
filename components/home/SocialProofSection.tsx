  ...
  return (
    <section className="py-24 bg-white overflow-hidden">
      <div className="text-center mb-16">
        <p className="text-sm font-bold text-blue-600 mb-3 tracking-widest uppercase">Trusted Partners</p>
      ...
      <div className="flex overflow-hidden mb-24">
        <div className="marquee-track animate-[marquee-infinite-left_20s_linear_infinite]">
          ...
          {[...companies, ...companies].map((item, i) => (
            <div key={`comp-${i}`} className="flex-none bg-slate-50 px-12 py-6 rounded-2xl border border-slate-100 text-slate-400 font-bold min-w-[200px] text-center shadow-sm">
              {item}
            </div>
          ))}
        </div>
      ...
      <div className="text-center mb-16">
        <h2 className="text-3xl font-bold text-slate-900">500명 이상의 크리에이터와 함께합니다</h2>
      </div>
        ...
        <div className="marquee-track animate-[marquee-infinite-right_22s_linear_infinite]">
          ...
          {[...creators, ...creators].map((item, i) => (
            <div key={`cre-${i}`} className="flex-none flex flex-col items-center gap-4 min-w-[160px]">
              ...
              <p className="text-sm font-bold text-slate-400">{item}</p>
            </div>
          ))}
        </div>
      </div>
